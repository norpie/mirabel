...
<div class="flex h-full flex-col">
    <div class="flex-none space-y-1 px-6 pb-4 pt-2">
        ...
        <p class="text-muted-foreground">Connect external services and manage API access.</p>
    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        {#each oauthConnections as connection}
                            <div class="text-xs text-muted-foreground">
                                {connection.provider} - Connected {formatDate(
            ...
            <Card.Header>
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <Card.Description>Manage API keys for programmatic access</Card.Description>
                    </div>
                        ...
                        {#each apiKeys as apiKey}
                            <div class="flex items-center justify-between rounded-lg border p-4">
                                <div class="space-y-1">
                                    ...
                                    <div class="text-xs text-muted-foreground">
                                        Created {formatDate(apiKey.createdAt)} • Last used {formatDate(
                                            apiKey.lastUsed
                                        )}
                                    </div>
                                </div>
        ...
        <Card.Root>
            ...
            <Card.Content class="space-y-4">
                {#if webhooks.length === 0}
                    <div class="py-8 text-center text-muted-foreground">
                        No webhooks configured yet. Create your first webhook to receive real-time
                        notifications.
                    </div>
                {:else}
                    <div class="space-y-3">
...
<Dialog.Root bind:open={showNewApiKeyDialog}>
    <Dialog.Content class="sm:max-w-md">
        ...
        <Dialog.Footer>
            ...
            <Button onclick={createApiKey} disabled={!newApiKeyName.trim() || isLoading}>
