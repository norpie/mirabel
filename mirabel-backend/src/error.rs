...
// Import for the ResponseError implementation
...
pub enum Error {
    ...
    // Mirabel Core
    #[error("Mirabel Core error: {0}")]
    MirabelCore(#[from] mirabel_core::Error),
...
// TODO: Temporary implementation of ResponseError for backend Error type
// This is needed during the migration to support actix-web response handling
// with ApiResponse. Should be reviewed and properly implemented later.
impl ResponseError for Error {
    fn error_response(&self) -> HttpResponse {
        let status = self.status_code();
        let error_message = self.to_string();
        let api_response = ApiResponse::<()>::error(status, error_message);
    ...
    fn status_code(&self) -> StatusCode {
        match self {
            ...
            Error::NotFound | Error::NotFoundRecentUpdate(_) => StatusCode::NOT_FOUND,
            Error::AlreadyExists(_) | Error::Conflict(_) => StatusCode::CONFLICT,
            Error::DoubleSubscription => StatusCode::CONFLICT,
