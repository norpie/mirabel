<script lang="ts">
	...
	import { Shield, ShieldCheck, Key, AlertTriangle, Copy, CheckCircle, QrCode } from 'lucide-svelte';
	...
	async function startSetup() {
		...
		try {
			...
			// Mock QR code URL
			qrCodeUrl = 'https://chart.googleapis.com/chart?chs=200x200&chld=M|0&cht=qr&chl=otpauth://totp/Mirabel:user@example.com?secret=JBSWY3DPEHPK3PXP&issuer=Mirabel';
			setupStep = 'verify';
		} catch (error) {
					...
					<Card.Content class="space-y-4">
						{#if setupStep === 'setup'}
							<div class="text-center">
								<div class="inline-block h-32 w-32 bg-muted rounded border flex items-center justify-center">
									Loading QR Code...
								</div>
							</div>
						{:else if setupStep === 'verify'}
							<div class="space-y-4">
								<div class="text-center">
									{#if qrCodeUrl}
										<img src={qrCodeUrl} alt="2FA QR Code" class="mx-auto h-40 w-40 border rounded" />
									{/if}
								</div>
					...
					<Card.Content class="space-y-4">
						...
						<div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
							{#each twoFactorStatus.backupCodes as code, index}
								<div class="flex items-center justify-between rounded border p-3 font-mono text-sm">
									<span>{code}</span>
									<Button
										variant="ghost"
										size="sm"
