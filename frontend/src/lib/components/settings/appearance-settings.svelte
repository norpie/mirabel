				...
				<Card.Content class="space-y-4">
					<div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
						{#each themes as theme (theme.value)}
							<button
								class="relative flex flex-col items-center space-y-3 rounded-lg border p-4 transition-colors hover:bg-muted {selectedTheme === theme.value ? 'border-primary bg-muted' : 'border-border'}"
								onclick={() => handleThemeChange(theme.value)}
							>
								<div class="flex h-12 w-12 items-center justify-center rounded-full bg-background border">
